		...
		try
		{
			if (_dotransactions)
			{
				...
				{       
                                        ...
                                        //boolean succ = false;
                                        //try {
                                        //    succ = _workload.doTransaction(_db, _workloadstate);
                                        //}
                                        //finally {
                                        //    if (succ) {
  ...
  public static final String MAX_EXECUTION_TIME = "maxexecutiontime";
	...
	public static void usageMessage()
	{
		...
		System.out.println("  -s:  show status during run (default: no status)");
                System.out.println("  -i:  instrument operations (default: no instrumentation)"); // option added by Muntasir
                System.out.println(" -clog: path to log file folder");  // option added by Muntasir.
			...
			throws IOException
	{
		...
		try
		{
			...
			String exporterStr = props.getProperty("exporter", "com.yahoo.ycsb.measurements.exporter.TextMeasurementsExporter");  // Use JSON instead of Text
                        
                        //if (instrument) {
                        //    exporterStr = props.getProperty("exporter", "com.yahoo.ycsb.measurements.exporter.JSONMeasurementsExporter");  // Use JSON instead of Text
                        //}
	...
	public static void main(String[] args)
	{
		...
		String label="";
                ...
                boolean instrument = false;  // added by Muntasir, true call instrumented versions of operations, otherwise call regular versions
		...
		while (args[argindex].startsWith("-"))
		{
			...
			else if (args[argindex].compareTo("-s")==0)
			{
				...
				argindex++;
                        ...
                        else if (args[argindex].compareTo("-i") == 0) { // added by Muntasir for instrumentation
                            instrument = true;
                            //props.setProperty("exportfile", "muntasir.log");
                            argindex++;
                            //if (argindex>=args.length)
				//{
				//	usageMessage();
				//	System.exit(0);
				//}
				//props.setProperty("i","true");
                                //logFilePath = args[argindex];
				//argindex++;
                        }
                        else if (args[argindex].compareTo("-clog") == 0) {
                            argindex++;
				if (argindex>=args.length)
				{
					usageMessage();
				...
				props.setProperty("clog",args[argindex]);
                                logFilePath = args[argindex];
				argindex++;
                        }
